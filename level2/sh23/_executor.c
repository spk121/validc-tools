...
void executor_destroy(Executor *exec) {
    ...
    // Note: func_store is owned by caller, not freed here
    free(exec);
}
...
// Apply a single redirect (unchanged from original)
...
// Expand a string with expansions (unchanged from original)
...
static ExecStatus exec_simple_command(Executor *exec, ASTNode *node) {
    ...
    if (argv && argv[0]) {
        ...
        if (strcmp(argv[0], "return") == 0) {
            ...
            int status = 0; // Default return status
            if (argc > 1) {
                char *endptr;
        ...
        if (func) {
            ...
            if (ret == EXEC_RETURN) {
                return EXEC_SUCCESS; // Convert return to success for caller
            }
            return ret;
        }
    }
...
// Execute a pipeline (unchanged from original)
...
// Pattern matching for case (unchanged from previous)
...
ExecStatus executor_run(Executor *exec, ASTNode *ast) {
    ...
    switch (ast->type) {
        ...
        case AST_FUNCTION_DEFINITION: {
            // Store function in func_store
            function_store_set(exec->func_store, ast->data.function_definition.name, ast->data.function_definition.body);
            executor_set_status(exec, 0);
