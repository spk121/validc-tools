...
static void prepare_undo(Editor *ed) {
    if (ed->undo_lines) {
        for (int i = 0; i < ed->undo_num_lines; i++) free(ed->undo_lines[i]);
        free(ed->undo_lines);
        ed->undo_lines = NULL;
    }
    ed->undo_num_lines = 0;
    ...
    if (ed->num_lines > 0) {
        ...
        if (!ed->undo_lines) critical_error(ed);
        for (int i = 0; i < ed->num_lines; i++) {
            ed->undo_lines[i] = my_strdup(ed->lines[i]);
            if (!ed->undo_lines[i]) critical_error(ed);
        }
...
static int search_pattern(Editor *ed, const char *pattern, size_t pat_len, bool forward) {
    if (ed->num_lines == 0) return 0;
    
    // Create null-terminated pattern
    char pat[MAX_LINE];
    if (pat_len >= sizeof(pat)) return 0;
    ...
    if (forward) {
        ...
        for (int idx = start + 1; idx < ed->num_lines; idx++) {
            if (bre_match(ed->lines[idx], pat, &m)) {
...
int parse_one_address(const char **pp, int current, int last_line, const int marks[26]) {
    ...
    // Optional + or -
    if (*p == '+') { p++; relative = true; }
    else if (*p == '-') { p++; sign = -1; relative = true; }
    ...
    if (*p == '$') {
        ...
        if (mark_idx < 0) return ADDR_ERROR;  // unmarked
 ...
 */
const char *parse_ed_address(const char *line,
                             int *addr1, int *addr2, bool *have_comma,
                             int current_line, int last_line, const int marks[26])
{
    const char *p = line;
    *addr1 = 0;
    *addr2 = 0;
...
void init_editor(Editor *ed) {
    ...
    ed->verbose = 0;
    ed->last_error = NULL;
    for (int i = 0; i < 26; i++) ed->marks[i] = -1;
    ed->prompt = 0;
    ed->undo_lines = NULL;
    ed->undo_num_lines = 0;
...
int parse_address(Editor *ed, const char *addr) {
    if (!addr || !*addr) return ed->current_line;
    
    // Trim leading/trailing spaces
    while (*addr == ' ' || *addr == '\t') addr++;
    if (*addr == '\0') return ed->current_line;
    ...
    if (*addr == '/' || *addr == '?') {
        ...
        if (*p != delim) return -1; // Unterminated pattern
        pattern[i] = '\0';
        ...
        if (found <= 0 || found > ed->num_lines) return -1;
...
AddressRange parse_address_range(Editor *ed, const char *range_str) {
    ...
    const char *after = parse_ed_address(range_str, &addr1, &addr2, &have_comma,
                                          current_1based, last_1based, ed->marks);
    ...
    // Validate range
    if (addr1 >= 0 && addr1 < ed->num_lines &&
        addr2 >= 0 && addr2 < ed->num_lines &&
        addr1 <= addr2) {
        result.start = addr1;
        result.end = addr2;
    }
...
void append_line(Editor *ed, int addr) {
    ...
    if (!input_fp) printf("(Enter text, end with '.' on a new line)\n");
    while (1) {
        int had_nl = 0;
...
void delete_line(Editor *ed, int addr) {
    ...
    if (ed->current_line < 0) ed->current_line = -1;  // Empty buffer
...
static void execute_command(Editor *ed, const char *cmd) {
    ...
    const char *cmd_start = parse_ed_address(cmd_buf, &addr1, &addr2, &have_comma,
                                               current_1based, last_1based, ed->marks);
    ...
    if ((op == 'g' || op == 'v') && cmd_start[1] == '/') {
        ...
        char pat[MAX_LINE] = {0}; size_t i = 0;
        while (*p && *p != '/' && i < sizeof(pat) - 1) pat[i++] = *p++;
        if (*p != '/') { set_error(ed, "Invalid global"); return; }
        pat[i] = '\0'; p++; // skip closing '/'
        // Skip leading spaces before inner command
        while (*p == ' ') p++;
        const char *inner = p; // command or '{' to begin a list
        if (!*inner) { set_error(ed, "Invalid global"); return; }
        ...
        int *idxs = (int*)malloc(cap * sizeof(int)); if (!idxs) critical_error(ed);
        int n = 0; BreMatch m;
        for (int i2 = r.start; i2 <= r.end; i2++) {
            int matched = bre_match(ed->lines[i2], pat, &m);
            if ((is_g && matched) || (!is_g && !matched)) idxs[n++] = i2;
        }
        ...
        if (inner[0] == '{' && inner[1] == '\0') {
            ...
            if (!list) { free(idxs); critical_error(ed); }
            ...
            while (fgets(linebuf, sizeof(linebuf), input_fp ? input_fp : stdin)) {
                ...
                *e = '\0';
                ...
                if (!stored) { for (size_t qi=0; qi<list_len; qi++) free(list[qi]); free(list); free(idxs); critical_error(ed); }
                if (list_len == list_cap) {
                    size_t new_cap = list_cap * 2;
                    char **new_list = (char**)realloc(list, new_cap * sizeof(char*));
                    if (!new_list) { for (size_t qi=0; qi<list_len; qi++) free(list[qi]); free(list); free(idxs); critical_error(ed); }
                    list = new_list; list_cap = new_cap;
                }
            ...
            for (int k = n - 1; k >= 0; k--) {
                ...
                while (ci < list_len) {
                    ...
                    if (!cmdline || cmdline[0] == '\0') { ci++; continue; }
                    ...
                    if (first_non_addr == 'i' || first_non_addr == 'a') {
                        ...
                        if (temp_input) {
                            ...
                            // Execute command
                            char tmp[MAX_LINE];
                            strncpy(tmp, cmdline, sizeof(tmp)-1); tmp[sizeof(tmp)-1] = '\0';
            ...
            for (size_t qi=0; qi<list_len; qi++) free(list[qi]);
            free(list);
            free(idxs);
            return;
        } else {
    ...
    if (op == 's' && cmd_start[1] == '/') {
        ...
        if (*p != '/') { set_error(ed, "Invalid substitute"); return; }
        pattern[i] = '\0';
        p++; // skip '/'
        i = 0;
        while (*p && *p != '/' && i < sizeof(replacement) - 1) { replacement[i++] = *p++; }
        if (*p != '/') { set_error(ed, "Invalid substitute"); return; }
        replacement[i] = '\0';
        p++; // skip '/'
        int global = 0;
        while (*p) { if (*p == 'g') global = 1; else { set_error(ed, "Invalid flag"); return; } p++; }
    ...
    if (op == 'e') {
        ...
        if (filename == NULL) filename = ed->filename;
        if (filename == NULL) {
            set_error(ed, "No current filename");
            return;
        }
    ...
    if (op == 'p' || op == 'd' || op == 'n' || op == 'l' || op == 'c' || op == 'j') {
        ...
        switch (op) {
            case 'p': print_range(ed, range); break;
            case 'd': delete_range(ed, range); break;
            case 'n': print_numbered_range(ed, range); break;
            case 'l': print_list_range(ed, range); break;
            case 'c': change_range(ed, range); break;
            case 'j': join_range(ed, range); break;
        }
        return;
    }
    ...
    switch (op) {
        case 'a': append_line(ed, addr < 0 ? (ed->num_lines > 0 ? ed->num_lines - 1 : -1) : addr); break;
        case 'i': insert_line(ed, addr < 0 ? 0 : addr); break;
        case 'u':
            if (!ed->undo_valid) { set_error(ed, "Nothing to undo"); break; }
            // Swap current buffer with undo snapshot
            {
                // Free current lines
                for (int i = 0; i < ed->num_lines; i++) free(ed->lines[i]);
                free(ed->lines);
        ...
        case 'P': ed->prompt = !ed->prompt; printf("Prompt %s\n", ed->prompt ? "on" : "off"); break;
...
void change_range(Editor *ed, AddressRange range) {
    ...
    if (insert_addr > ed->num_lines) insert_addr = ed->num_lines;
    if (insert_addr < 0) insert_addr = 0;
...
void move_range(Editor *ed, AddressRange range, int dest_addr) {
    ...
    if (!moved_lines) critical_error(ed);
...
void copy_range(Editor *ed, AddressRange range, int dest_addr) {
    ...
    if (!new_lines) critical_error(ed);
    ed->lines = new_lines;
    ...
    for (int i = 0; i < num_lines; i++) {
        ...
        if (src_idx > dest_addr) src_idx += num_lines;
        
        ed->lines[dest_addr + 1 + i] = my_strdup(ed->lines[src_idx]);
        if (!ed->lines[dest_addr + 1 + i]) critical_error(ed);
    }
...
void substitute_range(Editor *ed, AddressRange range, const char *pattern, const char *replacement, int global) {
    if (range.start < 0 || range.end < 0) { set_error(ed, "Invalid address"); return; }
    if (range.start >= ed->num_lines || range.end >= ed->num_lines) { set_error(ed, "Invalid address"); return; }
    if (!pattern || !replacement) { set_error(ed, "Invalid substitute"); return; }
    ...
    for (int j = range.start; j <= range.end; j++) {
        ...
        if (global) {
            ...
            if (!work) critical_error(ed);
            int changed = 0;
            while (bre_match(work, pattern, &m)) {
                char *next = bre_substitute(work, pattern, replacement);
                if (!next) { free(work); critical_error(ed); }
                if (strcmp(next, work) == 0) { free(next); break; }
                free(work);
                work = next;
                changed = 1;
            }
            ...
            if (bre_match(ed->lines[j], pattern, &m)) {
                char *new_line = bre_substitute(ed->lines[j], pattern, replacement);
                if (!new_line) critical_error(ed);
                free(ed->lines[j]);
                ed->lines[j] = new_line;
                any_changed = 1;
            }
    ...
    if (!any_changed) { set_error(ed, "No match"); return; }
    ed->dirty = 1;
    ed->current_line = range.end;  // 0-indexed
}
